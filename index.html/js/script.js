...
function fecharSelectsCustomizados(selectAtual = null) {
  document
    .querySelectorAll(".select-personalizado.aberto")
    .forEach((select) => {
...
function atualizarSelectCustomizado(select) {
  ...
  const opcoes = select.querySelectorAll(
    "[data-action='selecionar-opcao-custom']",
  );
  ...
  if (texto && opcaoSelecionada) {
    texto.textContent = opcaoSelecionada.textContent.trim();
  } else if (texto) {
    texto.textContent = select.dataset.placeholder || "";
  }
}
...
function selecionarOpcaoCustomizada(opcao) {
  ...
  if (texto) texto.textContent = opcao.textContent.trim();

  select
    .querySelectorAll("[data-action='selecionar-opcao-custom']")
    .forEach((item) => {
...
let vlibrasParentOriginal = null;
let vlibrasProximoElementoOriginal = null;
let observadorVLibrasModal = null;
const controlesVLibrasMovidos = [];

function ehControleInteracaoVLibras(elemento) {
  if (!(elemento instanceof HTMLElement)) return false;

  return elemento.textContent.trim() === "Interagir";
}

function posicionarControleInteracaoVLibras(controle, modal) {
  const margem = 12;
  const distanciaModal = 8;
  const areaModal = modal.getBoundingClientRect();
  const areaControle = controle.getBoundingClientRect();
  const largura = areaControle.width || 126;
  const altura = areaControle.height || 44;
  const esquerda = Math.min(
    window.innerWidth - largura - margem,
    Math.max(margem, areaModal.right - largura),
  );
  const abaixo = areaModal.bottom + distanciaModal;
  const acima = areaModal.top - altura - distanciaModal;
  const topo =
    abaixo + altura <= window.innerHeight - margem
      ? abaixo
      : Math.max(margem, acima);

  controle.style.position = "fixed";
  controle.style.left = `${esquerda}px`;
  controle.style.top = `${topo}px`;
  controle.style.width = `${largura}px`;
  controle.style.height = `${altura}px`;
  controle.style.zIndex = "2147483647";
  controle.style.pointerEvents = "auto";
}

function moverControlesInteracaoVLibras(modal) {
  if (!modal) return;

  document.body
    .querySelectorAll("button, a, [role='button'], [tabindex], div, span")
    .forEach((controle) => {
      if (!ehControleInteracaoVLibras(controle) || modal.contains(controle))
        return;

      controlesVLibrasMovidos.push({
        elemento: controle,
        parent: controle.parentNode,
        proximo: controle.nextSibling,
        estiloOriginal: controle.getAttribute("style"),
      });

      const posicao = controle.getBoundingClientRect();
      controle.classList.add("vlibras-interagir-modal");
      modal.appendChild(controle);
      controle.style.width = `${posicao.width}px`;
      controle.style.height = `${posicao.height}px`;
      posicionarControleInteracaoVLibras(controle, modal);
    });
}

function moverVLibrasParaModal(modal) {
  const vlibras = document.querySelector("[vw]");

  if (!modal || !vlibras || modal.contains(vlibras)) return;

  if (!vlibrasParentOriginal) {
    vlibrasParentOriginal = vlibras.parentNode;
    vlibrasProximoElementoOriginal = vlibras.nextSibling;
  }

  modal.appendChild(vlibras);
  modal.classList.add("modal-com-vlibras");
  moverControlesInteracaoVLibras(modal);

  observadorVLibrasModal = new MutationObserver(() => {
    if (modal.open) moverControlesInteracaoVLibras(modal);
  });

  observadorVLibrasModal.observe(document.body, {
    childList: true,
    subtree: true,
  });
}

function restaurarVLibras() {
  const vlibras = document.querySelector("[vw]");

  if (observadorVLibrasModal) {
    observadorVLibrasModal.disconnect();
    observadorVLibrasModal = null;
  }

  while (controlesVLibrasMovidos.length) {
    const controle = controlesVLibrasMovidos.pop();

    controle.elemento.classList.remove("vlibras-interagir-modal");
    if (controle.estiloOriginal === null) {
      controle.elemento.removeAttribute("style");
    } else {
      controle.elemento.setAttribute("style", controle.estiloOriginal);
    }

    if (controle.proximo && controle.proximo.parentNode === controle.parent) {
      controle.parent.insertBefore(controle.elemento, controle.proximo);
    } else if (controle.parent) {
      controle.parent.appendChild(controle.elemento);
    }
  }

  if (!vlibras || !vlibrasParentOriginal) return;

  if (
    vlibrasProximoElementoOriginal &&
    vlibrasProximoElementoOriginal.parentNode === vlibrasParentOriginal
  ) {
    vlibrasParentOriginal.insertBefore(vlibras, vlibrasProximoElementoOriginal);
  } else {
    vlibrasParentOriginal.appendChild(vlibras);
  }

  document.querySelectorAll(".modal-com-vlibras").forEach((modal) => {
    modal.classList.remove("modal-com-vlibras");
  });

  vlibrasParentOriginal = null;
  vlibrasProximoElementoOriginal = null;
}
...
function atualizarControlesFotoPerfil() {
  ...
  const temFoto =
    foto.getAttribute("src") && foto.classList.contains("foto-visivel");
...
function alternarMenuRapido(botao) {
  ...
  document
    .querySelectorAll(".navegacao-rapida.aberto")
    .forEach((menuAberto) => {
...
function carregarAlunoParaEdicao() {
  ...
  document.getElementById("dataNascimento").value = formatarData(
    aluno.dataNascimento,
  );
...
function validarCredencialForte(valor, descricaoCampo) {
  if (valor.length < 8) {
    return `${descricaoCampo} deve ter no mínimo 8 caracteres.`;
  }

  const numeros = valor.match(/\d/g) || [];
  if (numeros.length < 2) {
    return `${descricaoCampo} deve ter no mínimo 2 números.`;
  }

  const especiais = valor.match(/[!@#$%^&*(),.?":{}|<>_\-+=/\\[\];'~`]/g) || [];
  if (especiais.length < 1) {
    return `${descricaoCampo} deve ter no mínimo 1 caractere especial.`;
  }

  if (!/[A-Z]/.test(valor)) {
    return `${descricaoCampo} deve ter no mínimo 1 letra maiúscula.`;
  }

  if (!/[a-z]/.test(valor)) {
    return `${descricaoCampo} deve ter no mínimo 1 letra minúscula.`;
  }
...
function alterarSenha() {
  ...
  const erroForcaSenha = validarCredencialForte(novaSenha, "A nova senha");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".modal-saida").forEach((modal) => {
    modal.addEventListener("close", restaurarVLibras);
  });
...
function configurarNavegacaoRapida(paginaAtual) {
  ...
  const paginaNavegacao =
    paginaAtual === "relatorio-documento" ? "historico" : paginaAtual;

  document
    .querySelectorAll(".navegacao-rapida [data-nav-page]")
    .forEach((botao) => {
...
function salvarNovoCodigoSeguranca() {
  ...
  const erroForcaCodigo = validarCredencialForte(codigo1, "O novo código");
...
function gerarGraficoBarras(idElemento, dados, total) {
  ...
  if (total === 0) {
    area.innerHTML = "<p class='texto-centro'>Nenhum aluno cadastrado.</p>";
    return;
  }
...
function carregarGraficosRelatorio() {
  ...
  if (relatorioTotal) {
    relatorioTotal.innerText = formatarQuantidadeAlunos(total);
  }
  ...
  if (total === 0) {
    ...
    if (relatorioTotal) {
      relatorioTotal.innerText = "0 alunos";
    }
    return;
  }

  if (relatorioPagamento) {
    relatorioPagamento.innerText = maiorPagamento.nome;
  }

  if (relatorioPlano) {
    relatorioPlano.innerText = maiorPlano.nome;
  }
}
...
function carregarDocumentoRelatorio() {
  ...
  if (totalAlunos) {
    totalAlunos.innerText = formatarQuantidadeAlunos(total);
  }
  ...
  if (total === 0) {
    ...
    pagamento.innerText = maiorPagamento.nome;
  }

  if (total > 0 && plano) {
    plano.innerText = maiorPlano.nome;
  }
  ...
  if (tabela) {
    ...
    if (historico.length === 0) {
      ...
      historico.forEach((item, index) => {
        const linha = document.createElement("tr");
        adicionarCelulaTabela(linha, index + 1, "N°");
